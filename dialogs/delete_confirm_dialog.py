...
class DeleteConfirmDialog(PrototypeDialog):
    def __init__(
        ...
        *,
        ...
        missing_only = bool(records) and all(
            item.get("_file_state") == "missing" for item in records
        )
        ...
        body = QLabel(
            f"将从列表清理选中的 {len(records)} 条缺失音频记录；不会访问或删除磁盘文件，索引审计仍保留。"
            if live_mode and missing_only
            else f"将把选中的 {len(records)} 个音频文件移入应用备份目录，"
            "不会立即永久删除，可在操作历史中恢复。"
            if live_mode
        ...
        warning = QLabel(
            "该操作只隐藏已经确认缺失的列表记录；文件重新出现并被扫描后会恢复显示。"
            if live_mode and missing_only
            else "确认后会实际移动以上音频文件；已匹配歌词不会随音乐自动删除。"
            if live_mode
        ...
        self.backup_linked_lyrics.setVisible(live_mode and not missing_only)
        ...
        footer, _primary = footer_buttons(
            self,
            "清理记录" if live_mode and missing_only else "移入备份" if live_mode else "删除",
            danger=True,
        )
...
class DeleteLyricsConfirmDialog(PrototypeDialog):
    def __init__(
        ...
        *,
        ...
        body = QLabel(
            (
                f"将从列表清理选中的 {len(records)} 条缺失歌词记录；不会访问或删除磁盘文件，索引审计仍保留。"
                if missing_only
                else f"将检查选中的 {len(records)} 个歌词文件；仍被音乐引用的歌词会拒绝处理，"
                "其余文件将移入应用备份目录并可恢复。"
            )
            if live_mode
            else (
        ...
        warning = QLabel(
            "该操作只隐藏已经确认缺失的列表记录；文件重新出现并被扫描后会恢复显示。"
            if live_mode and missing_only
            else "确认后会实际移动未被引用的歌词文件，不会立即永久删除。"
            if live_mode
